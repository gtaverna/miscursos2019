...
public class NuevoGrupo extends HttpServlet {
		...
		try {
			...
			if (session != null) {
				...
				if (u != null) {
					...
					request.getRequestDispatcher("grupos/nuevo.jsp").forward(request, response);
		...
		try {
			...
			if (session != null) {
				...
				if (u != null) {
					...
					gc.setNombre(nombre);
					gc.setMarca(u.getMarca());
					gc.setActivo(true);
					if (new GestorGrupos().NuevoGrupo(gc)) {
