		...
		try {
			...
			if (session != null) {
				...
				if (u != null) {
					...
					request.getRequestDispatcher("usuarios/nuevo.jsp").forward(request, response);
		...
		try {
			...
			if (session != null) {
				...
				if (u != null) {
					...
					if (u.getApp_nivel() != Info.PERM_ADMIN) {
						throw new Exception("No tienes permiso para crear nuevos usuarios.");
					}
					...
					if ( new GestorUsuarios().NuevoUsuario(nu) ) {
